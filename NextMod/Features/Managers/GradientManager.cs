...
namespace NEXT.Features.Managers;
...
public static class GradientManager
{
    ...
    static GradientManager()
    {
        try
        {
            GradientConfig = PluginSingleton<NEXTPlugin>.Instance.Config.Bind("Gradient", "Secondary", 0, "Gradient ID");
        }
        catch (Exception e)
        {
            Logger<NEXTPlugin>.Warning(e.ToString());
        }
    }
