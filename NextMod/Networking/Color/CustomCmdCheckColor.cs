...
public class CustomCmdCheckColor(NEXTPlugin plugin, uint id) : PlayerCustomRpc<NEXTPlugin, CustomColorData>(plugin, id)
{
    ...
    private static bool VerifyColor(byte requestedColor, byte requestedGradient, byte playerId)
    {
        ...
        {
            Logger<NEXTPlugin>.Error($"Error getting gradient for player {data.PlayerName}");
